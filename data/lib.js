...
function slView(mode, flag){
	...
	if(mode == "save"){
		...
		$("#sl").click(function(){
			...
			try {
				// 从localStorage读取存档
				var saveData = localStorage.getItem("tinywaste_save_" + name);
				
				if (saveData) {
					if(flag != null){
						startGame();
						if(loadData(unescape(saveData))){
...
function save(overwrite, name){
	...
	// 使用localStorage替代PHP存档，适用于GitHub Pages
	try {
		// 保存到localStorage
		localStorage.setItem("tinywaste_save_" + name, savefile);
		
		// 保存用户名列表
		var savedUsers = localStorage.getItem("tinywaste_users") || "[]";
		var users = JSON.parse(savedUsers);
		if (users.indexOf(name) === -1) {
			users.push(name);
			localStorage.setItem("tinywaste_users", JSON.stringify(users));
		}
